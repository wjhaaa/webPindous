...
function buildColorSections(keyword = "") {
  ...
  MARD_COLORS.forEach((color) => {
    const searchText = [
      ...
      color.series,
      color.seriesLabel,
      SERIES_SEARCH_TERMS[color.series] || ""
    ].join(" ").toLowerCase();

    if (safeKeyword && !searchText.includes(safeKeyword)) {
      return;
    }

    if (!grouped.has(color.series)) {
      grouped.set(color.series, []);
    }
    grouped.get(color.series).push(color);
  });

  return Array.from(grouped.entries()).map(([series, colors]) => ({
    series,
